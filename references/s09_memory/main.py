...
def _parse_frontmatter(text: str) -> tuple[dict, str]:
    """解析 Markdown 文件的 YAML frontmatter，返回 (元数据字典, 正文内容)。"""
    # 检查是否包含 frontmatter 起始标记，若无则直接返回原文本
    ...
    meta = {}
    # 遍历 frontmatter 区域（分割后的第二部分）的每一行
    for line in parts[1].strip().splitlines():
        if ":" in line:
            # 以第一个冒号为界分离键和值，防止值内部包含冒号被错误拆分
            k, v = line.split(":", 1)
            # 清理键值两端的空白字符，并剥离可能存在的单/双引号包裹
            meta[k.strip()] = v.strip().strip('"').strip("'")
            
    # 返回提取的元数据字典，以及剩余正文内容（去除首尾空白）
    return meta, parts[2].strip()


def write_memory_file(name: str, mem_type: str, description: str, body: str):
    """Write a single memory file with YAML frontmatter."""
    # 将记忆名称清洗为安全的文件名字符串（转小写，用连字符替换空格和斜杠）
    slug = name.lower().replace(" ", "-").replace("/", "-")
    filename = f"{slug}.md"
    filepath = MEMORY_DIR / filename
    # 拼接 YAML frontmatter 与正文内容，并写入对应的 .md 文件
    filepath.write_text(
        f"---\nname: {name}\ndescription: {description}\ntype: {mem_type}\n---\n\n{body}\n"
    )
    # 重新构建 MEMORY.md 索引文件，确保新记忆能被系统快速检索
    _rebuild_index()
    return filepath
...
def _rebuild_index():
    ...
    扫描 .memory/ 目录中的所有 .md 记忆文件，从 YAML frontmatter 提取元数据，
    生成包含各记忆文件链接的 Markdown 索引。该索引会被注入到 SYSTEM prompt 中，
    供代理快速发现和选择相关记忆。
    """
    lines = []
    # 按文件名排序遍历所有 .md 文件
    for f in sorted(MEMORY_DIR.glob("*.md")):
        # 跳过索引文件自身，避免循环引用和重复处理
        if f.name == "MEMORY.md":
            continue
        
        # 读取文件内容（指定 UTF-8 编码确保跨平台兼容性）
        raw = f.read_text(encoding="utf-8")
        meta, body = _parse_frontmatter(raw)
        
        # 获取记忆名称：优先使用 frontmatter 中的 name 字段，否则使用文件名（不含扩展名）
        name = meta.get("name", f.stem)
        
        # 获取描述信息：优先使用 frontmatter 的 description 字段，
        # 若无则从正文第一行截取前 80 个字符作为简要描述
        desc = meta.get("description", body.split("\n")[0][:80])
        
        # 构建 Markdown 格式的索引行：- [记忆名称](文件名) — 描述摘要
        lines.append(f"- [{name}]({f.name}) — {desc}")
    
    # 拼接所有索引行为完整文本，确保非空时末尾带换行符
    index_content = "\n".join(lines) + "\n" if lines else ""
    # 将索引内容写入 MEMORY.md 文件
...
def select_relevant_memories(messages: list, max_items: int = 5) -> list[str]:
    """根据最近对话内容筛选相关的记忆文件。
    优先通过 LLM 匹配记忆名称与描述，若失败则降级为关键词匹配。
    """
    # 获取所有已保存的记忆文件列表
    ...
    # 1. 提取最近的用户对话文本作为上下文
    recent_texts = []
    # 从后往前遍历消息，最多获取 3 条用户消息
    for msg in reversed(messages):
        if msg.get("role") == "user":
            content = msg.get("content", "")
            # 兼容 Anthropic API 返回的 list 格式（包含多个 content block）
    ...
    try:
        ...
        # 使用正则提取返回内容中的 JSON 数组部分，兼容可能存在的多余文本
        ...
        # LLM 调用失败、超时或 JSON 解析异常时静默降级
        pass

    # 5. 降级方案：基于关键词的启发式匹配
    # 过滤出长度大于 3 的有效单词，去除停用词干扰
    keywords = [w.lower() for w in recent.split() if len(w) > 3]
    selected = []
    for f in files:
        # 在记忆名称和描述中查找是否包含任意关键词
